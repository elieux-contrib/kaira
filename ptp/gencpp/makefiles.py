...
def prepare_makefile(project, config, directory):
    ...
    config["include"].append(kaira_path(paths.GNULIB_INCLUDE_DIR))
    config["libdir"].append(kaira_path(paths.GNULIB_LIB_DIR))
    ...
    makefile.set("MPILIBS",
                 project.get_build_option("LIBS") + " "
                 + " ".join(add_prefix("-l", s) for s in config["libs"])
                 + " -lcailiempi -lpthread -lgnu -lws2_32")
